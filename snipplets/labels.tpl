  ...
  {% if show_labels %}
    ...
    {% else %}
      ...
      {% if product.compare_at_price or product.promotional_offer %}
        <div class="{% if not product.promotional_offer and product %}js-offer-label{% endif %} label label-accent {% if not product.promotional_offer %}label-circle{% endif %} lbl-desconto" {% if (not product.compare_at_price and not product.promotional_offer) or not product.display_price %}style="display:none;"{% endif %} data-store="product-item-{% if product.compare_at_price %}offer{% else %}promotion{% endif %}-label">
          ...
          {% else %}
            <span class="js-offer-percentage">
              {{ price_discount_percentage |round }}</span>%
          {% endif %}
        </div>
      {% endif %}
      {% if product.free_shipping %}
        <div class="label label-accent lbl-frete-gratis">frete <br>grátis</div>
      {% endif %}
    {% endif %}
  {% endif %}
</div>
